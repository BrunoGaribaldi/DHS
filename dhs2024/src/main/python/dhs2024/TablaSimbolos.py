...
from ID import ID
from ID import Variable
from ID import Funcion
class TablaSimbolos():
    ...
    def addidentificador(self,nombre,tipoDato,tipoID,argumentos): #tipo ID es cero entonces es variable
        ...
        if(tipoID == 0): #variable
            ...
            id = Funcion(nombre,tipoDato,1,0,argumentos)
        contexto.tabla.update({nombre:id})
    
    def buscarLocal(self,nombre): #retorna 0 si se puede usar, 1 sino
        if (self.contextos[-1].traerVariable(nombre))==None:
            return 1
        else:
            return 0
    
    def buscarGlobal(self, nombre): #retorna 0 si se puede usar, 1 sino
